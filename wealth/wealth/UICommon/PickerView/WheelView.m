...
- (void)layoutView:(UIView *)__view atIndex:(int)index animated:(BOOL)animated duration:(double)duration {
    ...
    __view.hidden = (fabs(angle) >= M_PI/2.0);
    ...
    void(^changeLayerLayout)(void) = ^(void) {
        ...
        if (index == self->currentIndex) {
            ...
            if ([self.delegate respondsToSelector:@selector(wheelView:didSelectedRowAtIndex:)]) {
                
                [self.delegate wheelView:self didSelectedRowAtIndex:[self->_originalPositionedViews indexOfObject:__view]];
            }
...
- (void)layoutView:(UIView *)__view byAngle:(double)angle animated:(BOOL)animated duration:(double)duration {
    ...
    __view.hidden = (fabs(_angle) >= M_PI/2.0);
