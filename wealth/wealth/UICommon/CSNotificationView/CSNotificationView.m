                    ...
                    duration:(NSTimeInterval)duration
{
    ...
    note.contentColor = contentColor;
...
- (void)setVisible:(BOOL)visible animated:(BOOL)animated completion:(void (^)())completion
{
    if (_visible != visible) {
        ...
        if (!self.superview) {
            ...
            if ( [self.parentViewController isKindOfClass:NSClassFromString(@"UIBaseViewController")] ) {
                UIBaseViewController *vc = (UIBaseViewController *)self.parentViewController;
                UITableView *tableView = vc.mainTableView;
                [vc.view insertSubview:self belowSubview:vc.navigationBarView];
                [vc.view sendSubviewToBack:tableView];
