...
- (id)initWithCGImageSource:(CGImageSourceRef)imageSource scale:(CGFloat)scale
{
    ...
    _scale = scale;
    readFrameQueue = dispatch_queue_create("com.puhui.gifreadframe", DISPATCH_QUEUE_SERIAL);
...
{
    if (self.images.count) {
        return [self.images.firstObject CGImage];
    } else {
...
{
    if (self.images.count) {
        return [self.images.firstObject imageOrientation];
    } else {
