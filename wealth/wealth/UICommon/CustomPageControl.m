...
typedef NS_ENUM(NSUInteger, CustomPageControlImageType) {
    CustomPageControlImageTypeNormal = 1,
    CustomPageControlImageTypeCurrent,
    CustomPageControlImageTypeMask
};

typedef NS_ENUM(NSUInteger, CustomPageControlStyleDefaults) {
    CustomPageControlDefaultStyleClassic = 0,
    CustomPageControlDefaultStyleModern
};

static CustomPageControlStyleDefaults _defaultStyleForSystemVersion;
...
+ (void)initialize
{
    ...
    if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
        _defaultStyleForSystemVersion = CustomPageControlDefaultStyleModern;
    } else {
        _defaultStyleForSystemVersion = CustomPageControlDefaultStyleClassic;
    }
}
...
{
    _numberOfPages = 0;
    _tapBehavior = CustomPageControlTapBehaviorStep;
...
#else
    [self setStyleWithDefaults:CustomPageControlDefaultStyleClassic];
#endif
    
    _alignment = CustomPageControlAlignmentCenter;
    _verticalAlignment = CustomPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
    ...
    CGFloat left = 0.0f;
    switch (_alignment) {
        case CustomPageControlAlignmentCenter:
            left = ceilf(CGRectGetMidX(rect) - (size.width / 2.0f));
            break;
        case CustomPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
    ...
    switch (_verticalAlignment) {
        case CustomPageControlVerticalAlignmentMiddle:
            top = CGRectGetMidY(rect) - (height / 2.0f);
            break;
        case CustomPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(CustomPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return;
    }
    ...
    switch (type) {
        case CustomPageControlImageTypeCurrent:
            dictionary = self.currentPageImages;
            break;
        case CustomPageControlImageTypeNormal:
            dictionary = self.pageImages;
            break;
        case CustomPageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:CustomPageControlImageTypeNormal];
    [self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:CustomPageControlImageTypeCurrent];;
    [self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:CustomPageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(CustomPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return nil;
    }
...
{
    return [self _imageForPage:pageIndex type:CustomPageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:CustomPageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:CustomPageControlImageTypeMask];
}
...
- (void)setStyleWithDefaults:(CustomPageControlStyleDefaults)defaultStyle
{
    switch (defaultStyle) {
        case CustomPageControlDefaultStyleModern:
            ...
            break;
        case CustomPageControlDefaultStyleClassic:
        default:
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CustomPageControlTapBehaviorJump == self.tapBehavior) {
