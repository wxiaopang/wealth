...
typedef NS_ENUM(NSUInteger, CustomPageControlAlignment) {
    CustomPageControlAlignmentLeft = 1,
    CustomPageControlAlignmentCenter,
    CustomPageControlAlignmentRight
};

typedef NS_ENUM(NSUInteger, CustomPageControlVerticalAlignment) {
    CustomPageControlVerticalAlignmentTop = 1,
    CustomPageControlVerticalAlignmentMiddle,
    CustomPageControlVerticalAlignmentBottom
};

typedef NS_ENUM(NSUInteger, CustomPageControlTapBehavior) {
    CustomPageControlTapBehaviorStep	= 1,
    CustomPageControlTapBehaviorJump
};
...
@property (nonatomic) CustomPageControlAlignment alignment					UI_APPEARANCE_SELECTOR; // deafult is Center
@property (nonatomic) CustomPageControlVerticalAlignment verticalAlignment	UI_APPEARANCE_SELECTOR;	// deafult is Middle
...
@property (nonatomic) CustomPageControlTapBehavior tapBehavior;	// CustomPageControlTapBehaviorStep provides an increment/decrement behavior exactly like UIPageControl. CustomPageControlTapBehaviorJump allows specific pages to be selected by tapping their respective indicator. Default is CustomPageControlTapBehaviorStep
...
// CustomPageControl mirrors UIPageControl's standard accessibility functionality by default.
// Basically, the accessibility label is set to "[current page index + 1] of [page count]".

// CustomPageControl extends UIPageControl's functionality by allowing you to name specific pages. This is especially useful when using
